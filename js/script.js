const RADIO_NAME = 'La Urban';

// Change Azuracast Stream URL Here, .
const URL_STREAMING = 'https://play.laurban.cl';

//API URL Azuracast Now Playing
const API_URL = 'https://radio.laurban.cl/api/nowplaying/laurban';
...
class Page {
    constructor() {
        ...
        this.refreshNextSong = function(song, artist, cover) {
            ...
            if (songElement) {
                songElement.className = 'animated flipInY text-uppercase';
                songElement.innerHTML = song;
            }
            if (artistElement) {
                artistElement.className = 'animated flipInY text-capitalize';
                artistElement.innerHTML = artist;
            }
            setTimeout(function () {
                songElement.className = 'text-uppercase';
                artistElement.className = 'text-capitalize';
            }, 2000);
        }
        ...
        this.refreshHistoric = function (songData, index) {
            ...
            if (index >= 0 && index < historicSongs.length) {
                var historicItem = historicSongs[0];
                var coverElement = historicItem.querySelector('.cover-historic');
                var songElement = historicItem.querySelector('.song');
                var artistElement = historicItem.querySelector('.artist');
...
function getStreamingData() {
    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            ...
            document.title = RADIO_NAME + ' | Sonando: ' + currentSong + ' - ' + currentArtist;

            // Atualiza as informações da música atual
            if (document.getElementById('currentSong').innerHTML !== currentSong) {
                page.refreshCover(currentSong, currentArtist);
                page.refreshCurrentSong(currentSong, currentArtist);
                page.refreshNextSong(data.playing_next.song.title, data.playing_next.song.artist, data.playing_next.song.art);
            }
...
function refreshLyric (currentSong, currentArtist) {
    function toPascalCase(str) {
        return str
            ...
            .map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()) // Convierte la primera letra de cada palabra a mayúscula y el resto a minúscula
            .join(' '); // Une las palabras de nuevo con espacios
    }
    var actualSong = document.getElementById('currentSong');
    var actualArtist = document.getElementById('currentArtist');
    const openLyric = document.getElementsByClassName('lyrics')[0];
    var song = actualSong.innerHTML;
    var artist = actualArtist.innerHTML;
    artist = artist.split(';')[0];
    song = song.split('Feat')[0];
    artist = toPascalCase(artist);
    song = toPascalCase(song);
